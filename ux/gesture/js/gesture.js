...
/**
 * @fileOverview Implements a Gesture management system. Determines whether some
 *               gestures (with exactly 2 fingers) are performed on the nodes
 *               and if so, inform the listeners. Gestures that can be listened
 *               are referenced in the "_knownGestures" attribute below. The
 *               notifications to the listeners include parameters that depends
 *               on gesture type.
 * 
...
define([ '../../../_amd/core' ], function(wink) 
{
	 ...
	 * @namespace
	 * 
	 * Implements a Gesture management system. Determines whether some
	 * gestures (with exactly 2 fingers) are performed on the nodes and
	 * if so, inform the listeners. Gestures that can be listened are
	 * referenced in the "_knownGestures" attribute below. The
	 * notifications to the listeners include parameters that depends on
	 * gesture type. <br>
	 * <br>
	 * The gesture object is able to determines whether some gestures
	 * (with exactly 2 fingers) are performed on the nodes and if so,
	 * inform the listeners. Gestures that can be listened are : <br> -
	 * two_digits_click: on click with 2 fingers <br> -
	 * two_digits_press: on pressure with 2 fingers <br> - enlargement:
	 * A separation of the 2 fingers, as if to zoom in <br> - narrowing:
	 * bring 2 fingers closer, as if to zoom out <br> - rotation: a
	 * rotation with 2 fingers <br> - instant_scale: Each alignment or
	 * each spacing of 2 fingers <br> - instant_rotation: Each rotation
	 * of 2 fingers <br> - gesture_end: the end a a gesture <br>
	 * <br>
	 * The notifications to the listeners include parameters that
	 * depends on gesture type.
	 * 
	 * @example
	 * 
	 * var node = wink.byId("nodeId"); wink.ux.gesture.listenTo(node,
	 * "two_digits_click", { context: window, method: "twoDigitsClick" }, {
	 * preventDefault: true });
	 * 
	...
	wink.ux.gesture = {
		_gestureElements : [],
		_knownGestures : [ "two_digits_click", // { digit1, digit2 }
		...
		listenTo : function(domNode, gesture, callback, options) {
			if (!this._isGestureKnown(gesture)) {
				wink
						.log('[gesture] Cannot listen to this unknown gesture: '
								+ gesture);
				return false;
			}
			...
			if (wink.isSet(options)) {
				if (options === true) {
					opts.preventDefault = true; // backwards
												// compatibility
				} else {
			...
			if (index == null) {
				gestureElement = this._createGestureElement(domNode,
						opts.preventDefault);
				...
				wink.ux.touch
						.addListener(
								gestureElement.domNode,
								"start",
								{
									context : this,
									method : this
											._getEvtKey("_handleStart"),
				...
				wink.ux.touch
						.addListener(
								...
								"gesturestart",
								{
									context : this,
									method : this
											._getEvtKey("_handleGestureStart"),
									arguments : [ gestureElement ]
								},
								{
									preventDefault : gestureElement.preventDefault
			...
			var gestureElement = {
				...
				removeCallback : function(gesture, callback) {
					if (this.isListening(gesture)) {
						...
						for ( var i = 0; i < this
								.getGestureHandler(gesture).callbacks.length; i++) {
							var callbackI = this
									.getGestureHandler(gesture).callbacks[i];
							if ((isFn && callbackI == callback)
									|| (!isFn
											&& callbackI.context == callback.context && callbackI.method == callback.method)) {
		...
		/**
		 * Invoke callbacks that are concerned by the gesture on the
		 * given element.
		 * 
		...
		_handleStart : function(uxEvent, gestureElement) {
			...
			if (nbTouches == 2) {
				...
				gestureElement.multitouchStartTime = this
						._getTimeStamp();

				gestureElement.digits.push(this._getDigit(
						uxEvent.srcEvent.targetTouches[0], uxEvent));
				gestureElement.digits.push(this._getDigit(
						uxEvent.srcEvent.targetTouches[1], uxEvent));
				...
				wink.ux.touch
						.addListener(
								...
								"move",
								{
									context : this,
									method : this
											._getEvtKey("_handleMove"),
									arguments : [ gestureElement ]
								},
								{
									preventDefault : gestureElement.preventDefault
								});
				wink.ux.touch.addListener(gestureElement.domNode,
						"end", {
							context : this,
				...
				gestureElement.checkTimer = wink.setTimeout(this,
						"_checkTwoDigitsPressed",
						this.TWO_DIGITS_PRESS_MIN_DURATION,
						gestureElement);
			} else {
				gestureElement.multitouch = false;
				wink.ux.touch.removeListener(gestureElement.domNode,
						"move", {
							context : this,
		...
		_PointerHandleStart : function(uxEvent, gestureElement) {
			...
			gestureElement.boxGesture
					.addPointer(uxEvent.srcEvent.pointerId);
			gestureElement.addDigit(this._getDigit(uxEvent.srcEvent,
					uxEvent));
		...
		_handleEnd : function(uxEvent, gestureElement) {
			if (gestureElement.multitouch == true) {
				...
				if (nbTouches == 0) {
					wink.ux.touch.removeListener(
							gestureElement.domNode, "move", {
								context : this,
								method : this._getEvtKey("_handleMove")
							});
					wink.ux.touch.removeListener(
							gestureElement.domNode, "end", {
								context : this,
								method : this._getEvtKey("_handleEnd")
							});

					gestureElement.multitouchEndTime = this
							._getTimeStamp();
					var multitouchDuration = gestureElement.multitouchEndTime
							- gestureElement.multitouchStartTime;
					...
					if (multitouchDuration < this.TWO_DIGITS_CLICK_MAX_DURATION) {
						...
						this._notifyGesture("two_digits_click",
								gestureElement, gestureInfos);
					}
				}
			}
		},
		...
		_checkTwoDigitsPressed : function(gestureElement) {
			...
			if (isValid) {
				...
				this._notifyGesture("two_digits_press", gestureElement,
						gestureInfos);
			}
		},
		/**
		...
		_handleGestureStart : function(uxEvent, gestureElement) {
			wink.ux.touch.addListener(gestureElement.domNode,
					"gesturechange", {
						context : this,
						method : this
								._getEvtKey("_handleGestureChange"),
						arguments : [ gestureElement ]
					}, {
						preventDefault : gestureElement.preventDefault
					});
		...
		_handleGestureChange : function(uxEvent, gestureElement) {
			if (gestureElement.multitouch == true) {
				var scaleValue = uxEvent.srcEvent.scale, rotateValue = uxEvent.srcEvent.rotation, scaleAmplitude = scaleValue, rotationAmplitude = rotateValue;
				...
				if (Math.abs(scaleAmplitude) > this.SCALE_MIN_VALUE) {
					...
					this._notifyGesture(currentGesture, gestureElement,
							scaleGestureInfos);
				}

				if (Math.abs(rotationAmplitude) > this.ROTATION_MIN_VALUE) {
					gestureElement.rotation = rotateValue;
					this._notifyGesture("rotation", gestureElement,
							rotationGestureInfos);
				}

				this._notifyGesture("instant_scale", gestureElement,
						scaleGestureInfos);
				this._notifyGesture("instant_rotation", gestureElement,
						rotationGestureInfos);
			}
		},
		/**
		...
		_handleGestureEnd : function(uxEvent, gestureElement) {
			...
			var gestureDuration = gestureElement.gestureEndTime
					- gestureElement.gestureStartTime;
