...
public abstract class nsPSMDetector {
	...
	public nsPSMDetector(final int aItems, final nsVerifier[] aVerifierSet, final nsEUCStatistics[] aStatisticsSet) {
	...
	protected void initVerifiers(final int currVerSet) {
		...
		// ==========================================================
			...
			mVerifier = new nsVerifier[] { new nsUTF8Verifier(), new nsSJISVerifier(), new nsEUCJPVerifier(), new nsISO2022JPVerifier(), new nsEUCKRVerifier(), new nsISO2022KRVerifier(), new nsBIG5Verifier(), new nsEUCTWVerifier(), new nsGB2312Verifier(), new nsGB18030Verifier(),
					new nsISO2022CNVerifier(), new nsHZVerifier(), new nsCP1252Verifier(), new nsUCS2BEVerifier(), new nsUCS2LEVerifier() };
			...
			if (((aLastChance && mSampler.GetSomeData()) || mSampler.EnoughData()) && (eucNum == possibleCandidateNum)) {
				...
				for (j = 0; j < mItems; j++) {
					if ((null != mStatisticsData[mItemIdx[j]]) && (!(mVerifier[mItemIdx[j]].charset()).equals("Big5"))) {
						final float score = mSampler.GetScore(mStatisticsData[mItemIdx[j]].mFirstByteFreq(), mStatisticsData[mItemIdx[j]].mFirstByteWeight(), mStatisticsData[mItemIdx[j]].mSecondByteFreq(), mStatisticsData[mItemIdx[j]].mSecondByteWeight());
	...
	public String[] getProbableCharsets() {
		...
		if (mItems <= 0) {
			final String[] nomatch = new String[1];
			nomatch[0] = "nomatch";
			return nomatch;
		}

		final String ret[] = new String[mItems];
		for (int i = 0; i < mItems; i++) {
